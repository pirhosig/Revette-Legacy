	...
	for (int j = 0; j < atlasHeight; ++j)
	{
		for (int i = 0; i < atlasWidth; ++i)
		{
			...
			for (int lineCount = 0; lineCount < TEXTURE_SIZE; ++lineCount)
			{
				...
				auto lineBegin = imageDataArray + ((linePosition * imageWidth) + (i * TEXTURE_SIZE)) * TEXTURE_PIXEL_BYTE_SIZE;
				auto lineEnd = lineBegin + TEXTURE_SIZE * TEXTURE_PIXEL_BYTE_SIZE;
				auto textureDataPosition = textureDataArray + TEXTURE_SIZE * lineCount * TEXTURE_PIXEL_BYTE_SIZE;
